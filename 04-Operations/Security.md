...
OpenClaw 運用では、**ローカルGatewayの実際のlisten先を `openclaw gateway status` で確認する**。
固定の旧ポートやUI用ポートを疎通確認先にすると、Gatewayは正常でも誤検知になることがある。

### 推奨確認
```bash
openclaw gateway status
openclaw status
curl -fsS http://127.0.0.1:18789/ >/dev/null && echo "GW:OK" || echo "GW:DOWN"
```

### 補足
- `openclaw gateway status` を正本とする
- Browser debug port（9222/9224/9227 など）は `127.0.0.1` bind を確認する
- 週次チェックの疎通先は、Mission Controlや別UIのポートと混同しない
